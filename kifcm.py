...
def calculate_membership_degrees(V, X, m=2, alfa=150):
    c = V.shape[0]
    n = X.shape[0]
    U = np.zeros((n, c))
    D = np.zeros((n, c))
...
def update_cluster_centers(U, X, f):
    c = U.shape[0]
    n = X.shape[0]
...
def Kifcm(X_ifs, c, ε, m, alfa):
    V = initialize_seeds(c, X_ifs)
    k = 0
    perf = []
    for i in range(50) :
        U, D = calculate_membership_degrees(V, X_ifs, m, alfa)
...
def graph_init( X , c, alfa):
    X_ifs = convert_dataset_to_ifs(X)
    V_final, weight, D, perf=Kifcm(X_ifs,c , ε, m, alfa)
    distance_centers =[]
