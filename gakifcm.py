...
def calculate_membership_degrees(V, X,alfa, m):
    c = V.shape[0]
    n = X.shape[0]
    U = np.zeros((n, c))
    D = np.zeros((n, c))
...
def check_stopping_criterion(V_old, V_new, ε, alfa):
    c = V_old.shape[0]
    d = 0
    for i in range(c):
        d += 1-d1(V_old[i], V_new[i], alfa)
    return d / c < ε
...
def ga_kifcm(X_ifs, c, ε, m,alfa):
    V = initialize_seeds(c, X_ifs)
    k = 0
    perf = []
    g = 0
    for i in range(50):
        U, distance_matrix = calculate_membership_degrees(V, X_ifs,alfa,m)
        V_new = update_cluster_centers(U, X_ifs)
        """if check_stopping_criterion(V, V_new ,ε, alfa):
            g = check_stopping_criterio_obj(V, V_new,ε, alfa)
            break"""
        perf.append(perfor(X_ifs , U ,distance_matrix , c, m))
        V = V_new
        k += 1
    
    return V, np.transpose(U) , 6, distance_matrix, perf
...
def check_stopping_criterio_obj(V_old, V_new, ε, alfa):
    c = V_old.shape[0]
    d = 0
    for i in range(c):
        d += 1-d1(V_old[i], V_new[i], alfa)
...
def fitness_function(X,F,c, lamda, m, alfa):
    ...
    V , weight ,objective,distance_matrix, perf = ga_kifcm(X_ifs, c, ε, m,alfa)
...
def gakifcm(X,F,c,m_start,m_end,lamda_start,lamda_end ,n_population , n_generations,alfa):
    prob_crossover = 0.8
    prob_mutation = 0.2
    ...
    for i in range(n_generations):
        #weights[0] = lamda
        #weights[1] = m
        fitt = [fitness_function(X,F,c ,weights[0], weights[1],alfa ) for weights in population]
...
def graph_init(X,c,F,m_start,m_end,lamda_start,lamda_end,n_population , n_generations,alfa):
    lamda , m , V_final, weight, distance_matrix, perf = gakifcm(X,F,c,m_start,m_end,lamda_start,lamda_end,n_population , n_generations,alfa)
