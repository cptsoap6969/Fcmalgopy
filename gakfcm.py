...
def kfcm_obj(X,C ,c ,m, alfa):
    weight, distance_matrix, perf = kfcm(X,C ,c ,m, alfa)
...
def fitness_function(X,C ,c, m, alfa):
    objective = kfcm_obj(X ,C,c ,m, alfa)
    return objective
...
def ga_kfcm(X,c,n_population , n_generations, alfa):
  prob_crossover = 0.7
  prob_mutation = 1.0
  ...
  for i in range(n_generations):
      fitt = [fitness_function(X,centers ,c, m, alfa) for centers in population]
...
def kfcm(X, C , c, m, alfa):
    ...
    for it in range(50): # Total number of iterations
        ...
        for i in range(0, n):
            temp_array = []
            for j in range(0, c):
                x =  1 if 1-distance(X[i], C[j], alfa) == 0 else 1-distance(X[i], C[j], alfa)
                temp_array.append( x )
            distance_matrix.append(temp_array)
        ...
        """if check_stopping_criterion(C, C_new ,ε, alfa):
            break"""
        C = C_new
...
def graph_init(X,c,n_population , n_generations, alfa):
    C , weight, distance_matrix, perf = ga_kfcm(X,c,n_population , n_generations, alfa)
    # weight, distance_matrix, perf = kfcm(X,C ,c ,m)
    distance_centers =[]
...
def distance(point, center,alfa = 150):
...
def check_stopping_criterion(V_old, V_new, ε, alfa):
    ...
    d = 0
    for i in range(c):
        d += distance(V_old[i], V_new[i], alfa)
    return d / c < ε